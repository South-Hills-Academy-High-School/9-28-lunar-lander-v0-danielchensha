...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    Shark.ay = 25 * Math.sin(angle)
    Shark.ax = 25 * Math.cos(angle)
...
scaling.scaleByPercent(Shark, -25, ScaleDirection.Uniformly, ScaleAnchor.Middle)
Shark.ay = 20
...
game.onUpdate(function () {
    ...
    minimap.includeSprite(myMinimap, Shark)
    minimap2 = sprites.create(minimap.getImage(myMinimap), SpriteKind.map)
    minimap2.setPosition(Shark.x - 50, Shark.y - 30)
    engine.setPosition(Shark.x + -8 * Math.cos(angle), Shark.y + -8 * Math.sin(angle))
    engine.setPosition(Shark.x + -8 * Math.cos(angle), Shark.y + -8 * Math.sin(angle))
    fireball.setPosition(Shark.x + -8 * Math.cos(angle), Shark.y + -8 * Math.sin(angle))
})
